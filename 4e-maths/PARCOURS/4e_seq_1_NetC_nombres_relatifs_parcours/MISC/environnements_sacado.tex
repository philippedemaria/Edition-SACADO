...
\coordinate(NO) at (-2,-.5);
\coordinate(SW) at (22,1.5);
\coordinate(titre) at (0,0);
\coordinate(nom) at (12,.3);
\coordinate(prenom) at (12,-.3);
%\coordinate(qr) at (16.85,0.);
\shade[left color=#2 , right color=#2 ] (NO) rectangle (SW);
\draw (titre) node[color=texte, anchor=west]{ {\Large \bf  #1} \quad\quad \bf {\large \titreChap} };
\shade[left color=#2!10 , right color=#2!10 ] (12,-0.5) rectangle (18,1.5);
\draw (nom) node[color=black,anchor=west] {\large \bf Nom : \ldots\ldots\ldots\ldots\ldots\ldots };
\draw (prenom) node[color=black,anchor=west] {\large \bf Prénom : \ldots\ldots\ldots\ldots\ldots };
\end{tikzpicture}
}
