...
%\begin{tikzpicture}
%	\tkzTabInit{$x$ / 1 , $f(x)$ / 2}{$-5$, $0$, $3$, $+\infty$}
%	\tkzTabVar{+/$4$, -/$1$,  +/$6$,  -/$-\infty$}
%\end{tikzpicture}
%
%	\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=3cm,y=0.1cm]
%		\draw [xstep=1.5cm,ystep=1cm, dash dot] (0,0) grid (4,44);
...
%		\draw[->,color=black] (0,0) -- (0,42);
%		\foreach \y in {0,10,20,30,40}
%		\draw[shift={(0,\y)},color=black] (2pt,0pt) -- (-2pt,0pt) node[left] {\footnotesize $\y$};
%		\draw[color=black] (8.1,0.5) node[right] {\footnotesize $0$};
%		\clip(-0.1,-2) rectangle (4.3,46);
%		\draw [color=red] (0,0)--(1,13)--(2,25)--(3,36)--(3.75,42);
%		\fill [color=red] (3.75,42) circle (2pt);
%		\draw [color=blue] (0,0)--(1,10)--(2,28)--(3,37)--(3.5,42);
%		\fill [color=blue] (3.5,42) circle (2pt);
%		\draw (4.08,0) node[anchor=center] {h};
%		\draw (0,44) node[anchor=center] {km};
%		
%\end{tikzpicture}
%
...
%            xtick={-6,-5,...,1},
%            ytick={-2,-1,...,4},]
%            \clip(-6.1,-2.1) rectangle (1.1,4.1);
%            %\draw[line width=2pt,smooth,samples=100, domain=-6.1:1] plot(\x,{1/2*((\x)^2+6*\x+5)});
%        \end{axis}
%\end{tikzpicture}
%
%\begin{tikzpicture}[scale=0.7]
%				\tkzTabInit{$x$ / 1 , $f(x)$ /1.5}{$-\infty$, $\ldots\ldots$,  $+\infty$}
%				\tkzTabLine{,,,,}
%			\end{tikzpicture}
...
%	%y tick style={none}
%	]
%	\clip(-4.1,-1.2) rectangle (4.1,2.2);
%	\draw[line width=1.5pt,color=red,smooth,samples=20,domain=0:4.1] 
%				plot(\x,{cos(\x*60)/2+1.5});
%	\draw (3.9,1.5) node[anchor=center] {$\red \mathscr{C}_f$};
%	\draw[line width=1.5pt,color=blue,smooth,samples=20,domain=0:4.1] 
%				plot(\x,{sin(\x*90)});
%	\draw (3.9,0.5) node[anchor=center] {$\blue \mathscr{C}_g$};
%	\end{axis}
%\end{tikzpicture}
...
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45]
	\begin{axis}[
		...
		xtick={-4,-2,...,4},
		ytick={-0.5,0,...,1},]
		\clip(-4.1,-0.5) rectangle (4.1,1.2);
		\draw[line width=1pt,color=red,smooth,samples=100,
		domain=-4.1:4.1] plot(\x,{1/(1+(\x)^2)});
		\draw (3.9,0.25) node[anchor=center] {$\red \mathscr{C}_f$};
	\end{axis}
\end{tikzpicture}

\begin{center}\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45]
    \begin{axis}[
        x=1cm,y=0.4cm,
        axis lines=middle,
        ymajorgrids=true,
        ...
        xtick={-3,-2,...,5},
        ytick={-2,0,...,9},]
        \clip(-3.2,-4.2) rectangle (5.2,9.2);
        \draw[line width=2pt,color=red,smooth,samples=100,
        domain=-3.2:5.2] plot(\x,{(\x)^2-(\x)});
        \draw[color=red] (-1.7,6) node[anchor=center] {\red $\mathscr{C}_g$};
        \draw[line width=2pt,color=blue    ,smooth,samples=100,
        domain=-3.2:5.2] plot(\x,{-(\x)^2+3*(\x)+6});
        \draw[color=blue] (4.3,2.4) node[anchor=center] {\blue $\mathscr{C}_f$};
        \end{axis}
\end{tikzpicture}
\end{center}
